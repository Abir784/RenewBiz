...
$join_product_order_table="SELECT o.id as order_id,p.image as product_image,p.name as product_name, o.created_at as order_date ,o.status as order_status From product p,orders o WHERE (o.product_id = p.id and o.user_id = $user_id)";
        ...
        <tbody id="Abir">
            <?php foreach($join_product_order_table_result as $orders){ ?>
                <tr>
                    ...
                    <?php }?>
                    <td><a href="../dashboard/generate_order_receipt.php?id=<?=$orders['order_id']?>">Download Order Reciept</a></td>
                </tr>
            <?php }?>
        </tbody>
    </table>
...
<script>
        $(document).on('change','#status',function() {
            ...
            $.ajax({
                type: 'POST',
                url: 'filter_products_status.php',
                data: {status: status},
